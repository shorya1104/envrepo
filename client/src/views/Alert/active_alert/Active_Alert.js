...
import { AddComment, Commentlist } from "@mock-api/data/datatable";
...
const Table = ({ tableInstance, className }) => {
  const { getTableProps, headerGroups, rows, getTableBodyProps, prepareRow, page,
  } = tableInstance;
...
console.log(SocketIo.connected);
  const handlePagination = (current) => {
    ...
    if (isConnected) {
      SocketIo.emit('onactivedata', ({ userid: DEFAULT_USER.id == null ? sessionStorage.getItem("user_id") : DEFAULT_USER.id, currentPage: current, limit: limit, areanumber: areanumber, devicestatus: devicestatus }));
    }
  };

  const saveComment = () => {
    if (commentDescription.length !== 0) {
      AddComment({ device_id: deviceId, user_id: DEFAULT_USER.id == null ? sessionStorage.getItem("user_id") : DEFAULT_USER.id, comment_text: commentDescription }, res => {
        if (res.data.success == true) {
          toast("comment add successfully", { toastId: 1 })
          setShow2(false)
        }
      })
    }
  ...
  const handleShow = (deviceid) => {
    setShow(true)
    Commentlist({ device_id: deviceid, user_id: DEFAULT_USER.id == null ? sessionStorage.getItem("user_id") : DEFAULT_USER.id }, res => {
  ...
  useEffect(() => {
    ...
    if (isConnected) {
      ...
      SocketIo.emit('onactivedata', ({ userId: DEFAULT_USER.id == null ? sessionStorage.getItem("user_id") : DEFAULT_USER.id, currentPage: currentPage, limit: limit, areanumber: areanumber, devicestatus: devicestatus }));
  ...
  const columns = React.useMemo(() => {
    return [
      ...
      { Header: 'Device Name', accessor: 'devicename', sortable: true, headerClassName: 'text-muted text-small text-uppercase w-10 px-3', cellClassName: 'text-alternate' },
      {
      ...
      },
      {
        ...
        sortable: true, headerClassName: 'text-muted text-small text-uppercase w-10 px-5', cellClassName: 'text-alternate'
      },
      {
      ...
      },
      {
        ...
        headerClassName: 'text-muted text-small text-uppercase w-10 px-2', cellClassName: 'text-alternate'
      }
      // {
      //   Header: 'Action',
      //   accessor: '',
