...
const Createteam = () => {
    ...
    const [allotedOpt, setAllotedOpt]       = useState([]);
                            ...
                            <Form onSubmit={handleSubmit}>
                                <Row className="g-3 pt-5 pb-5">
                                    ...
                                    <Col lg="4" className="mx-auto">
                                        ...
                                        <div>
                                            <Select className="react-select-container" name="location_id" classNamePrefix="react-select" onChange={(val) => {
                                                formik.setFieldValue("location_id", val.value);
                                                formik.setFieldValue("location_index", val.value);
                                            }} options={areaOptions} value={areaOptions[ values.location_index ]} />
                                            {errors.location_id && touched.location_id && (
                                                <div className="d-block invalid-tooltip"> {errors.location_id} </div>
                                            )}
                                        </div>
                                    </Col> 
                                    <Col lg="4" className="mx-auto">
                                        ...
                                        <div>
                                            <Select className="react-select-container" name="add_member" multiple="multiple" classNamePrefix="react-select" options={memberOptions} isMulti closeMenuOnSelect={false} onChange={(val) => {
                                                formik.setFieldValue("members", val);
                                                setAllotedOpt(val);
                                            }} value={allotedOpt} />
                                            {errors.members && touched.members && (
                                                <div className="d-block invalid-tooltip"> {errors.members} </div>
                                            )}
                                        </div>
                                    </Col>
                                    <Col lg="4" className="mx-auto">
                                        ...
                                        <div>
                                            <Select className="react-select-container" classNamePrefix="react-select" options= {memberOptions} value={memberOptions[ values.team_leader_index ]}
                                            onChange={(val) => {
                                                index = memberList.findIndex((object) => { // 
                                                    return object.member_id === val.value;
                                                });
                                                formik.setFieldValue("team_leader", val.value);
                                                formik.setFieldValue("team_leader_index", val.value);
